 ...
 * 
 * Source Author https://github.com/daentech/CVRP
 *****************************************************************************/
...
public class TimeGraph {
	...
	public void render(){
		...
		for(ResultSet result : results){
			...
			if(withKey){
				g.drawLine(topRightX + 30, (int)((height - (results.size() + 1) * 30) /2 + ((colorOffset + 1.5) * 20) - 5), topRightX + 20, (int)((height - (results.size() + 1) * 30) /2 + ((colorOffset + 1.5) * 20) - 5));
				g.setColor(Color.black);
				g.drawChars(result.getName().toCharArray(), 0, result.getName().length(), topRightX + 30, (int)((height - (results.size() + 1) * 30) /2 + ((colorOffset + 1.5) * 20)));
			}
